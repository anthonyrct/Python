...
desempenho_produto_data = [
    {'produto': 'Banheira', 'vendas_totais': '26 unidades', 'receita_total': 50000.00},
    {'produto': 'Ofurô', 'vendas_totais': '60 unidades', 'receita_total': 37200.00},
    {'produto': 'Spa', 'vendas_totais': '21 unidades', 'receita_total': 105000.00},
]
...
# Adicionando uma coluna 'mes' para facilitar a agregação
vendas_df['mes'] = vendas_df['data'].dt.month

# Contagem de vendas por mês
vendas_por_mes = vendas_df.groupby('mes').size()

# Contagem de vendas por mês
vendas_por_mes = vendas_df.groupby('mes')['quantidade'].sum()

# Preencher os meses sem vendas com zero
...
plt.xticks(range(1, 13))  # Definindo os rótulos do eixo x como os números dos meses de 1 a 12
...
estatisticas_descritivas.plot(kind='bar', color=cores)
plt.title('Estatísticas Descritivas (maximo, Minimo, etc..)')
...
# Calculando o número total de vendas de cada produto
produtos_mais_vendidos = vendas_df['produto'].value_counts()
...
# Plotando o gráfico de barras mostrando os produtos mais vendidos
...
# Plotando o gráfico de pizza mostrando a distribuição da receita entre os produtos
